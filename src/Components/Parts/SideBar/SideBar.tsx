...
const SideBar = memo(() => {
    ...
    return (
        <>
            <div
                className={`py-6 relative h-screen bg-gradient-to-b slate-700 to-slate-950 hidden lg:block
                ${isOpen ?
            ...
            <div className="w-[95%] fixed bottom-4 left-1/2 -translate-x-1/2 rounded-full  bg-zinc-950/40 backdrop-blur-sm px-4 py-2  inset-shadow shadow shadow-blue-950  lg:hidden z-[99] border border-slate-950 ">
                <div className="flex gap-2 items-center justify-around                
                    ">
                    {SideBar_Data.map(i => (
                        <Link key={i.id} to={i.path} className="flex flex-col justify-center items-center gap-1 mx-[1px]">
                            <i.icon className="text-xl text-slate-200" ></i.icon>
                            <h3 className="font-bold text-sm text-white  lg:text-2xl">{i.text}</h3>
                        </Link>
                    ))}
                </div>
            </div>
