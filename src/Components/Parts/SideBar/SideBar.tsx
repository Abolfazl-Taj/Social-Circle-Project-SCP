...
const SideBar = memo(({ isOpen, setIsopen }: {
    ...
    return (
        <div className={`py-6 shadow rounded bg-slate-700/20 relative  h-[100vh]  ${isOpen ? "w-[350px]" : "w-[100px]  px-8 flex flex-col"} overflow-hidden transition-all`}>
            ...
            <ul className="my-5">
                {SideBar_Data.map(Item => (
                    <li key={Item.id}>
                        <NavLink to={Item.path} className={`flex items-center w-full  text-gray-400 my-3   ${isOpen ? "justify-start px-4" : "justify-center rounded-full  w-full"}`}>
                            <i className={isOpen ? "text-3xl" : "text-2xl w-full flex justify-center"}><Item.icon /></i>
