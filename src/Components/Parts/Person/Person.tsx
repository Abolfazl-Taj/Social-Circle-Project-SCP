...
const Person = ({ id, Full_Name, Brith_Day, Description, Status, personality, traits }: personType) => {
  ...
  return (
    <div className="bg-zinc-950/70 border border-zinc-600 rounded-2xl py-6 px-8 shadow-lg transition-all  hover:scale-[1.02] duration-1000  ease-in-out overflow-x
    w-full md:w-1/2 lg:w-1/3 xl:w-[35%]  flex flex-col gap-y-4 animate-in fade-in zoom-in backdrop-blur-2xl PersonShadow">
      <div className="flex items-center justify-between">
        ...
        <Link className="flex text-white font-bold  pb-2 " to={`edit-person/${id}`}>
          <i className="text-[20px] transition-all hover:text-blue-500 hover:scale-105 text-white"><FiEdit /></i>
        </Link>
      </div>
      ...
      <section className="mt-4">
        <div className="flex items-center justify-around gap-3 bg-[#111] rounded-md px-4 py-2">
      ...
      <section className="flex flex-col items-center">
        <section className="my-1 w-full flex flex-col items-start gap-4">
          <div className="flex items-center">
            ...
            <span className="bg-red-700/40 py-[2px] px-4 rounded-full text-white font-bold">
          ...
          <ul className="flex flex-wrap gap-4">
            {traits?.map(trait => (
              <li
                className="bg-zinc-900  font-semibold  px-2 py-1 rounded-2xl text-white shadow border border-zinc-800/30 hover:zinc-blue-500/60 hover:shadow-zinc-500/20 transition-all duration-200 cursor-default"
                key={trait}
              >{trait}</li>
            ))}
          </ul>
